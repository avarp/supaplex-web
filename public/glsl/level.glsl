...
vec4 drawGameField(vec2 screenPos) {
  ...
  float tileIndex = floor(tileData.r * 256.0);
  vec2 frame = vec2(
    floor(tileData.g * uAnimationFramesTextureSize.x), 
    floor(tileData.b * uAnimationFramesTextureSize.y)
  );
  ...
  if (frame.x > 0.0 || frame.y > 0.0) {
    vec4 frameColor = texture2D(
      ...
      vec2(
        (frame.x + mod(gameFieldPos.x, TILE_SIZE)) / uAnimationFramesTextureSize.x,
        (frame.y + mod(gameFieldPos.y, TILE_SIZE)) / uAnimationFramesTextureSize.y
      )
    );
    // Green color is used as a masking color (a.k.a "Green screen")
    if (!(frameColor.r == 0.0 && frameColor.g == 1.0 && frameColor.b == 0.0)) {
      return frameColor;
    }
  }
